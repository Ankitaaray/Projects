...
const getUserReservation=async(req,res)=>{
    ...
    const reservations=await Reservation.get_reservation_by_user(userID)
    res.status(200).json(reservations)
};
const createReservation=async(req,res)=>{
    const userID=req.user.id;
    const {ResourceId, startTime, endTime}=req.body;
    const resource=await Resource.get_resource(ResourceId)
    if(!resource){
        return res.status(404).json({msg:'Resource not found'});
    }
    const reservation=await Reservation.create_reservation(ResourceId, userID, startTime, endTime);
    res.status(200).json(reservation)
};
const deleteReservation=async(req,res)=>{
    const userID=req.user.id;
    const rID=req.params.id
    const reservation=await Reservation.delete_reservation_by_user(rID,userID)
    if(!reservation){
